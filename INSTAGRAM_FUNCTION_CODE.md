# Instagram Comments Scraper - Supabase Function Code

Use a DIFFERENT environment variable name so it doesn't conflict with other scrapers!
...
const INSTAGRAM_APIFY_ACTOR_ID = Deno.env.get("INSTAGRAM_APIFY_ACTOR_ID") ?? "YOUR_INSTAGRAM_ACTOR_ID_HERE";
...
serve(async (req) => {
  ...
  const postUrl = normalizePostUrl(body.handle ?? "");
  const runIdFromClient = body.runId?.trim();

  // 1) START RUN (no runId)
  if (!runIdFromClient) {
    if (!postUrl) {
      return json({ error: "Missing or invalid Instagram post URL." }, 400);
    }

    const encodedActorId = encodeURIComponent(INSTAGRAM_APIFY_ACTOR_ID);
    const startRunUrl = `https://api.apify.com/v2/acts/${encodedActorId}/runs`;

    // Instagram comments scraper typically needs post URLs
    // Adjust inputPayload based on your actor's requirements
    const inputPayload = {
  ...
  const comments = items.map((item: any) => {
    ...
    return {
      ...
      timestamp: item.timestamp || item.createdAt || item.createTime || "",
  ...
  return csvResponse(csv, `instagram_comments_${postUrl.replace(/[^a-zA-Z0-9]/g, "_")}.csv`);
});
```

## Environment Variables to Set:

For the `instagram-comments-scraper` function, set:
- `APIFY_API_TOKEN` = your Apify token (same as other scrapers)
- `INSTAGRAM_APIFY_ACTOR_ID` = your Instagram actor ID (replace `YOUR_INSTAGRAM_ACTOR_ID_HERE` above)

**Note:** You may need to adjust the `inputPayload` structure based on your specific Instagram actor's requirements. Common variations:
- Some actors use `postURLs` instead of `startUrls`
- Some actors use `postUrls` (singular)
- Some actors need different parameter names

Check your actor's documentation and adjust the `inputPayload` accordingly.
