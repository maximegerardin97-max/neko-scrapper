# Fixed TikTok Comments Scraper - Supabase Function Code

The actor expects `postURLs` not `startUrls`. Here's the corrected code:
...
serve(async (req) => {
  ...
  if (!runIdFromClient) {
    ...
    // FIXED: Use postURLs instead of startUrls
    const inputPayload = {
      postURLs: [videoUrl],  // <-- Changed from startUrls to postURLs
      maxComments: 10000,
    };
